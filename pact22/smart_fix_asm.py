...
#                    fline_no += 1
                    if not fl_fragment:
                        m = re.match( "<\+\d+>\s+kernel_spmv_fragment_%d:$" % fragment, fl )
...
def smart_insertion( asm_path, dump_path, outf=sys.stdout ):
    ...
    for pos,l in enumerate(fasm):
        if fragment is None:
            m = re.match( "kernel_spmv_fragment_(\d+):$", l ) 
            if m is not None:
                ...
                while not in_position:
                    ...
                    m = re.match( "([0-9a-fA-F]+) <kernel_spmv_fragment_%d>:" % fragment, fl )
                    if m is not None: 
                        in_position=True
